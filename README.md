...
部分版本中，“删除项目”区域的项目名称和活跃会话数量是在弹窗打开后动态插入的，可能出现下面这种英文残留：

![项目设置动态摘要未翻译的复现截图](./showimg/project-settings-summary-bug.png)

本次修复覆盖了拆分文本节点、延迟插入的大型弹窗子树，以及项目列表批量渲染等场景，并保留代码块和编辑器区域的翻译隔离。可以使用下面的命令运行回归测试：
